...
!     *    on Friday, April 13, 2012 at 16:58:14
!     *    
!     * interface: basicsrv 
!     *    
!     ************************************
...
!      CALL dce_parse_arg_ref( CMDLINE, ODE_FILE,ODE_SERVER, RV)
      ...
      DECLARE INTEGER dce_result
           LET strng= SPACE$(100)
           LET CBUFFER$= "x" + STRING$(1%, 0%) 
           CALL dce_pop_int_vms_ref(DCE_TABLE, CBUFFER$, x)
           LET CBUFFER$= "y" + STRING$(1%, 0%) 
           CALL dce_pop_int_vms_ref(DCE_TABLE, CBUFFER$, y)
           dce_result = add &
               (    &
               x,    &
               y    &
      ...
      DECLARE INTEGER dce_result
           LET strng= SPACE$(100)
           LET CBUFFER$= "s1" + STRING$(1%, 0%) 
           D_MAGS = 50
           CALL dce_pop_char_str_vms_ref   &
                 (DCE_TABLE, CBUFFER$,   &
                s1, D_MAGS)
           dce_result = lower2upper &
               (    &
               s1,    &
               s2    &
               )
           CBUFFER$ = 's2' + STRING$(1%, 0%)
           D_MAGS = 50
...
!          
       ...
       IF (COUNTER = 1) THEN
          CALL Radd(DCE_TABLE, SOCKET)
       ELSE      
!            
!          
!
!          
       LET FUNC$ = "lower2upper"
       COUNTER= POS(DCE_FUNC$, FUNC$, 1) 
       IF (COUNTER = 1) THEN
          CALL Rlower2upper(DCE_TABLE, SOCKET)
       ELSE      
!            
!          
